...
/**
 * Get arbitrary session data by key
 */
export async function getSessionData(sessionId, key) {
    const conversation = await getConversation(sessionId);
    return conversation[key] || null;
}

/**
 * Update arbitrary session data by key
 */
export async function updateSessionData(sessionId, key, value) {
    const client = await clientPromise;
    const db = client.db(DB_NAME);
    const collection = db.collection(COLLECTION);
