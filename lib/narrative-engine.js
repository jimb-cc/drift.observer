...
export function parseDSL(source) {
    ...
    function parseInlineTag() {
        const token = advance();
        switch (token.name) {
            case 'pause': return { type: NodeType.PAUSE, ms: token.attrs.ms ? parseInt(token.attrs.ms) : null, line: token.line };
            case 'br':
            case 'blank': return { type: NodeType.TEXT, value: '\n', line: token.line };
    ...
    async runNode(node, state) {
        switch (node.type) {
            ...
            case NodeType.TEXT:
                ...
                return true;
            case NodeType.PAUSE:
                await this.onOutput({ type: 'pause', ms: node.ms });
                return true;
