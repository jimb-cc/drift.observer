...
export class NarrativeRuntime {
    ...
    async processInput(state, input) {
        ...
        if (prompt.capture) state.variables[prompt.capture] = input;
        ...
        try {
            const response = await fetch('https://api.anthropic.com/v1/messages', {
                ...
                body: JSON.stringify({
                    ...
                    max_tokens: 100,
                    system: `You classify player responses. Respond with ONLY: IDEAL, IDEAL:path_name, NEAR, or FAIL. Be generous - lean toward IDEAL/NEAR for thoughtful engagement.`,
                    messages: [{ role: 'user', content: evalPrompt }],
                }),
            });

            if (!response.ok) return { classification: 'near', path: null };
            ...
            if (result.startsWith('IDEAL:')) {
                ...
                return { classification: 'ideal', path: matching?.path || null };
            }
            if (result.startsWith('IDEAL')) return { classification: 'ideal', path: null };
            if (result.startsWith('FAIL')) return { classification: 'fail', path: null };
            return { classification: 'near', path: null };
        } catch (e) {
